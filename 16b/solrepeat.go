...
func main() {
	...
	prev := make(map[[proglen]ProgType]int)
	start := time.Now()
	haveloop := false
	for round := 0; round < 1*1000*1000*1000; round++ {
		if v, exist := prev[prog]; exist && !haveloop {
			haveloop = true
			fmt.Printf("round %d = round %d\n", round, v)
			round = 1000000000 - (1000000000 % round)
			fmt.Printf("jump to round %d\n", round)
		} else {
			prev[prog] = round
		}
		for _, op := range ops {
